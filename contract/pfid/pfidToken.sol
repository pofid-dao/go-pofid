...
contract pfidToken is SeroInterface,Ownable{
    using SafeMath for uint256;
    
    string private _name = "PFID";
    
    string private _symbol = "PFID";
    
    uint8 private _decimals = 18;
    
    string private _descp;
    
    uint256 private _totalSupply = 10000000 * 10 ** uint256(_decimals);
    
    bool private _canMint = true;
    
    address private _blackHoleContract;
     ...
     */
    constructor(address blackHoleContract) public payable{
        require(sero_issueToken(_totalSupply,_symbol));
        _blackHoleContract = blackHoleContract;
    }
    ...
    function blackHoleContract() public view returns(address){
        return _blackHoleContract;
    }
    ...
    function closeMint() public onlyOwner{
        require(_canMint,"has closed");
        _canMint = false;
    }
    
    function mint(uint256 amount) public onlyOwner {
        require(_canMint,"mint closed");
        require(sero_issueToken(amount,_symbol));
        _totalSupply = _totalSupply.add(amount);
    }
    
    function withDraw(uint256 amount) public onlyOwner {
        uint256 _balance = sero_balanceOf(_symbol);
        require(_balance >= amount,"balance not enough");
        require(sero_send_token(owner,_symbol,amount),"withDraw failed");
    }
    
    function burned() public payable {
        string memory _currency = sero_msg_currency();
        uint256 amount = msg.value;
        require(_utilCompareInternal(_currency , _symbol),"invalid currency");
        require(sero_send_token(_blackHoleContract,_currency,amount),"burned failed");
        _totalSupply = _totalSupply.sub(amount);
    }
    
    function _utilCompareInternal(string a, string b) internal pure returns (bool) {
